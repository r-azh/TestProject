...
def group_url_tasks(urls, result_dict, html_link_regex):
    try:
...
if __name__ == '__main__':
    ...
    urls.put('http://search.yahoo.com/')
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=number_of_cpus) as group_link_processes:
        for i in range(urls.qsize()):
            group_link_processes.submit(group_url_tasks, urls, result_dict, html_link_regex)
