...
model.save('C:\\_Data_\\topc\ML_data\\ch04\\wiki\\wiki_hdp.pkl')

# model = gensim.models.ldamodel.LdaModel.load('C:\\_Data_\\topc\ML_data\\ch04\\wiki\\wiki_hdp.pkl')


print('############### create topics ################')
topics = numpy.zeros((len(mm), len(model.get_topics())))
...
numpy.save(f'{path}{os.sep}topics_hdp.npy', topics)

# topics = numpy.load(f'{path}{os.sep}topics_hdp.npy')
...
create_word_claod(words, 'wiki_word_cloud_most_talked_aboud_hdp.png', 120, fontname='Cardo')
words = model.show_topic(weights.argmin(), 64)
create_word_claod(words, 'wiki_word_cloud_least_talked_aboud_hdp.png', 250)
