...
def get_duplicate_files(dir1, dir2=None):
    ...
    for f1 in d1_contents:
        ...
        if (file_extension, file_size) not in d1_extensions.keys():
            d1_extensions[(file_extension, file_size)] = [f1_full_name]
        else:
            d1_extensions[(file_extension, file_size)].append(f1_full_name)
    if dir2:
    ...
    if files_list:
        for f in files_list:
            if os.path.exists(f):
                ...
                os.remove(f)
        print('removed {} files :)'.format(len(files_list)))
